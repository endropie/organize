...
<div class="flex flex-auto">
    <div
        ...
        <div class="absolute bottom-0 right-0 md:top-0 w-28 h-28" style="z-index: -1">
            {{-- PERSENTATION BAR --}}
        ...
        <div class="flex items-center">
            <div class="flex flex-col">
                <div class="font-bold tracking-wider uppercase text-md text-secondary">Pendapatan VS Pengeluaran</div>
        ...
        <div class="flex flex-row flex-wrap mt-4 -mx-6">
            <div class="flex flex-col pl-4 my-3 ml-4 border-l-4 border-green-600 xs:w-full">
                ...
                <div class="w-40 mt-2 text-xl font-medium leading-none">
                    IDR <span class="@if($income < 0) text-red-600 @endif">{{ number_format($income, 0) }}</span>
                </div>
            </div>
            <div class="flex flex-col pl-4 my-3 ml-4 border-l-4 border-red-600 xs:w-full">
                <div class="text-xs font-semibold leading-none tracking-wider uppercase text-hint">
                    Pengeluaran</div>
                <div class="w-40 mt-2 text-xl font-medium leading-none">
                    IDR <span class="@if($expense < 0) text-red-600 @endif">{{ number_format($expense, 0) }}</span>
                </div>
            </div>
            <div class="flex flex-col pl-4 my-3 ml-4 border-l-4 border-gray-900 xs:w-full">
                ...
                <div class="w-40 mt-2 text-xl font-medium leading-none">
                    IDR <span class="@if($summary < 0) text-red-600 @endif">{{ number_format($summary, 0) }}</span>
                </div>
            </div>
        </div>
    </div>
